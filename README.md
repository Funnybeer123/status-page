...
- Anonymous **Report a problem** form (`POST /` or `POST /api/reports`). Creates an operator-only report (title, body, optional public component ids, status). It is not a public incident, does not change component status, and is hidden from `/`. Reports persist to gitignored `data/reports.json`. Rate-limit keys are hashed (raw IP is never stored). The operator UI shows the hashed key only. The public page does not list reports.
...
- Problem reports: persisted in gitignored `data/reports.json` (title, body, public component ids, status; hashed rate-limit key, never a raw IP). StatusViewer can list; StatusOperator can promote one to a public incident (public component ids only; internals rejected). Promote audit actor is `api-key` or Entra `oid`.
