...
import { diedInYear, diedThatYearHeading, parseAliveYear } from "@/lib/aliveWhen";
import { lifespan } from "@/lib/dates";
import { hideMinorDetails } from "@/lib/privacy";

export default async function DiedThatYearPage({
  searchParams,
}: {
  ...
  const people = (await prisma.person.findMany({ where: { familyId: ctx.family.id, deletedAt: null } }))
    .filter((person) => diedInYear(person, year) && !hideMinorDetails(ctx.role, person));
  return (
    <AppShell>
      <p className="font-sans text-xs uppercase tracking-[0.2em] text-gold">{ctx.family.name}</p>
      <h1 className="mt-2 font-display text-4xl" data-testid="died-year-heading">
        {diedThatYearHeading(year, people.length)}
      </h1>
      <p className="mt-3 text-bark">
        <Link href={`/tree/when?year=${year}`} className="text-seal">Who was alive in {year}</Link>
      </p>
      <ul className="mt-10 space-y-3" data-testid="died-year-list">
        {people.map((person) => (
        ...
        {!people.length ? <li className="text-bark">{diedThatYearHeading(year, 0)}</li> : null}
      </ul>
    </AppShell>
  );
}
