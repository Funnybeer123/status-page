...
export function ClippingForm({ people }: { people: { id: string; displayName: string }[] }) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/clippings", { method: "POST", body: data });
    const payload = await response.json();
    setBusy(false);
    if (!response.ok) {
      setError(payload.error || "Could not save that clipping.");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="paper-card mt-8 grid gap-3 p-5" data-testid="clipping-form">
      ...
      <fieldset className="font-sans text-sm">
        <legend className="mb-2">Named in the notice</legend>
