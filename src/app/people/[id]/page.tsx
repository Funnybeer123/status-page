      ...
      <div className="grid gap-8 md:grid-cols-[280px_1fr]">
        <aside className="paper-card overflow-hidden">
          ...
          <div className="p-5">
            ...
            {!hideChild ? (
              <Link href={`/people/${person.id}/card`} className="mt-2 block font-sans text-sm text-seal" data-testid="index-card-link">
                Index card
              </Link>
            ) : null}
            {!hideChild ? (
              <Link href={`/people/${person.id}/filmstrip`} className="mt-2 block font-sans text-sm text-seal" data-testid="filmstrip-link">
                Photograph filmstrip
              </Link>
            ) : null}
            {!hideChild ? (
              <Link href={`/people/${person.id}/lane`} className="mt-2 block font-sans text-sm text-seal" data-testid="memory-lane-link">
                Memory lane
              </Link>
            ) : null}
            {!hideChild ? (
